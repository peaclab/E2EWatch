...
class Configuration:
    ...
    def _set_directories(self):
        ...
        if self._conf['system'] == 'eclipse':
            ...
            if 'hdf_data_path' not in self.overrides:
                
                #if the data is collected with per-second granularity                    
                if self._conf['granularity'] == 0: 
                    self._conf['hdf_data_path'] = Path('/projectnb/peaclab-mon/aksar/datasets/eclipse_final_hdfs')
                else:
                    self._conf['hdf_data_path'] = Path('/projectnb/peaclab-mon/aksar/datasets/eclipse_final_minute_hdfs')                 
                logging.info('Using default HDF path, please verify')
            else:
                self._conf['hdf_data_path'] = self.overrides['hdf_data_path']
                
            self._conf['output_dir'] = Path('/projectnb/peaclab-mon/aksar/models/DeploymentModels') / self._conf['system']     
    ...
    def _prepare_directories(self):
        ...
        if not self._conf['model_config'] is None:
            ...
            self._conf['tuning_dir'] = self._conf['model_config_dir'] / 'tuning'
            if not self._conf['tuning_dir'].exists():
                self._conf['tuning_dir'].mkdir(parents=True)
